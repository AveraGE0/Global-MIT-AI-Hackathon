"""Module for getting instagram trend data"""
import json
import os
...
def load_instagram_hashtag_data(file_path: str) -> List[Dict[str, str]]:
    """
    Load Instagram hashtag data from a JSON file.
    ...
    try:
        ...
        for i, entry in enumerate(data):
            ...
            if "hashtag" not in entry or "posts" not in entry:
                logger.warning("Skipping invalid entry at index %d: Missing required keys", i)
                continue

            validated_data.append(
                {"hashtag": entry["hashtag"], "posts": entry["posts"]}
            )
...
def get_instagram_trending_hashtags(
    ...
    """
    Get trending Instagram hashtags from a JSON file with optional filtering.
    ...
    Returns:
        List of dictionaries containing hashtag information with keys:
        - hashtag: The hashtag name (without # symbol)
        - posts: The original post count string (e.g., "1.835B")
        - post_count: The numeric post count as an integer
    """
    hashtag_data = load_instagram_hashtag_data(file_path)
    hashtag_data.sort(key=lambda x: x["posts"], reverse=True)
